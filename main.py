...
def predict(request: PredictionRequest):
    try:
        ...
        # Đảm bảo tensors có hình dạng phù hợp
        # if X_test_rt_1.ndimension() != 3 or X_test_rt_2.ndimension() != 3:
        #     raise ValueError("Input tensors must be 3-dimensional.")
        
        # logger.info(f"Input shapes: X_test_rt_1={X_test_rt_1.shape}, X_test_rt_2={X_test_rt_2.shape}")

        with torch.no_grad():
            Y_pred = model(X_test_rt_1, X_test_rt_2).cpu().numpy()

        return {"prediction":labels[int(np.argmax(Y_pred))]}
