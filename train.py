...
# from scipy.stats import levy_l_gen
...
def main():
    ...
    model.eval()
    with torch.no_grad():
        Y_pred = model(X_0_t.to(device), X_1_t.to(device)).cpu().numpy()
    Y_test = Y_t.numpy()
    ...
    # Lưu confusion matrix vào file riêng
    plt.savefig(str(savedir / "confusion_matrix.png"))
    plt.close()
